...
make_request "POST" "/signup" "{\"username\": \"$ADMIN_USER\", \"password\": \"$ADMIN_PASS\"}" ""
echo "----------------------------------------------------------------------"

# 2. Signin Admin
echo "[2] Signing in Admin User..."
ADMIN_TOKEN=$(curl -s -X POST "$BASE_URL/signin" -H "Content-Type: application/json" -d "{\"username\": \"$ADMIN_USER\", \"password\": \"$ADMIN_PASS\"}")
echo "Admin Token received."
echo "----------------------------------------------------------------------"

# 3. Assign ADMIN Role to Admin User
echo "[3] Assigning ADMIN role to $ADMIN_USER..."
make_request "POST" "/role_create" "{\"username\": \"$ADMIN_USER\", \"roleName\": \"ADMIN\"}" "$ADMIN_TOKEN"
echo "----------------------------------------------------------------------"

# 4. Signup Normal User
echo "[4] Signing up Normal User ($NORMAL_USER)..."
make_request "POST" "/signup" "{\"username\": \"$NORMAL_USER\", \"password\": \"$NORMAL_PASS\"}" ""
echo "----------------------------------------------------------------------"

# 5. Signin Normal User
echo "[5] Signing in Normal User..."
USER_TOKEN=$(curl -s -X POST "$BASE_URL/signin" -H "Content-Type: application/json" -d "{\"username\": \"$NORMAL_USER\", \"password\": \"$NORMAL_PASS\"}")
echo "User Token received."
echo "----------------------------------------------------------------------"

# 6. Assign USER Role to Normal User (Using Admin Token)
echo "[6] Assigning USER role to $NORMAL_USER..."
make_request "POST" "/role_create" "{\"username\": \"$NORMAL_USER\", \"roleName\": \"USER\"}" "$ADMIN_TOKEN"
echo "----------------------------------------------------------------------"

# 7. Admin Creates Product
echo "[7] Admin creating a product (Laptop)..."
PRODUCT_RESP=$(curl -s -X POST "$BASE_URL/product" \
    -H "Content-Type: application/json" \
...
if [ ! -z "$PRODUCT_ID" ]; then
    ...
    make_request "PUT" "/product/$PRODUCT_ID" "{\"name\": \"Gaming Laptop\", \"price\": 1500.00}" "$ADMIN_TOKEN"
    echo "----------------------------------------------------------------------"
